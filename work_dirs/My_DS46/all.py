...
train_options = {
    ...
    'train_list_path': 'datalists/train_list_small4.json',
    'val_path': 'datalists/val_list4.json',
    ...
    'chart_loss': {  # Loss for the task
        ...
        'SOD': {
            ...
            'ignore_index': 255,
            'gamma': 1.5,
            'weight': [1.0, 1.0, 1, 1.5, 1.0],  # per-class alpha，5 classes (0-4)
        },
        'FLOE': {
    ...
    'down_sample_scale': 4,  # 训练降采样10倍：80m→800m，场景预加载入RAM，裁剪速度极快
    'val_freq': 1,   # scale=10时验证场景小（直接整场景推理），可每epoch验证
    'val_downsample_scale': 4,  # 设为1：验证自动沿用down_sample_scale=10，与训练分辨率一致
    ...
    'water_patch_max_ratio': 0.70,   # 水体占有效像素 > 70% 时触发拒绝
    'water_rejection_prob': 0.90,    # 触发后以 90% 的概率丢弃该 patch
    'rare_sampling_classes': [2],  # 新冰/幼冰(1, 合并后) 为稀有类目标
    'rare_sampling_alpha': 0.5,       # 0=均匀采样, 1=完全按稀有类密度采样， 设 rare_sampling_alpha=0.0 或 rare_sampling_classes=[] 可关闭此过滤。
