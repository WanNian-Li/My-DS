...
train_options = {
    ...
    'val_path': 'datalists/val_list1.json',
    ...
    'chart_loss': {  # Loss for the task
        ...
        'SOD': {
            ...
            'ignore_index': 255,
            'gamma': 1.5,
            'weight': [1, 1, 1, 1.8, 1],  # CE per-class weight: boost class3 (thick FYI)
        },
        'FLOE': {
    ...
    # --- 稀有类加权采样 ---
    'rare_sampling_classes': [1, 2, 3, 4],  # 薄一年冰(2)和厚一年冰(3)均为稀有类目标，缓解类3欠采样
    'rare_sampling_alpha': 0.1,       # 0=均匀采样, 1=完全按稀有类密度采样， 设 rare_sampling_alpha=0.0 或 rare_sampling_classes=[] 可关闭此过滤。
