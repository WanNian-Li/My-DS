...
train_options = {
    ...
    'chart_loss': {  # Loss for the task
        ...
        'SOD': {
            ...
            'ignore_index': 255,
            'gamma': 1.5,
            'weight': [1.0, 1.3, 1.1, 1.8, 1.0],  # per-class alpha，5 classes (0-4)
        },
        'FLOE': {
    ...
    'seed': 10,
    'epochs': 200,
    'epoch_len': 70,  # scale=10时场景预加载入内存，裁剪极快，可采样更多batch
    ...
    'water_patch_max_ratio': 0.80,   # 水体占有效像素 > 70% 时触发拒绝
    'water_rejection_prob': 0.90,    # 触发后以 90% 的概率丢弃该 patch
    'rare_sampling_classes': [2, 3],  # 新冰/幼冰(1, 合并后) 为稀有类目标
